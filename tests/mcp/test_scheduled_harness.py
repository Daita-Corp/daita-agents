...
from tests.support.mcp_actions import ActionModel, response
...
async def test_scheduled_mcp_harness_measures_runtime_and_preserves_failure_evidence(
...
async def test_scheduled_mcp_harness_reaches_both_occurrences_with_one_approval(
    tmp_path,
    fail_confirmation,
    profile_name,
    monkeypatch,
...
def test_scheduled_mcp_live_provider_requires_explicit_authorization(monkeypatch):
...
@pytest.mark.parametrize("streaming", [False, True])
async def test_scheduled_mcp_provider_failure_retains_unknown_usage_and_closes_provider(
    tmp_path,
    streaming,
):
...
def test_scheduled_mcp_live_budget_rejects_unbounded_values(monkeypatch, value):
    monkeypatch.setenv(COST_ENV, value)
    with pytest.raises(ValueError):
        limits()


def test_scheduled_mcp_matrix_and_repetitions_are_bounded(monkeypatch):
...
def test_scheduled_mcp_explicit_profile_preserves_strict_defaults(
...
async def test_scheduled_mcp_user_flow_prompts_and_review_remain_separate(
    tmp_path, monkeypatch
):
    from tests.support.mcp_routine_harness import REPORT_INSTRUCTION, report_path
