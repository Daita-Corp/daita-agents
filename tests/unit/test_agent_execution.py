...
def _make_focus_tool(name: str = "query_data") -> AgentTool:
    """AgentTool that has a `focus` parameter in its schema (simulates a DB plugin tool)."""
    ...
    return AgentTool(
        name=name,
        description="Query data with optional focus filtering",
        parameters={
            "type": "object",
            "properties": {
                ...
                "focus": {
                    "type": "string",
                    "description": "Focus DSL to pre-filter results",
                },
            },
            "required": ["sql"],
        },
...
class TestAutoFocus:
    def _get_system_content(self, llm) -> str:
        messages = llm.call_history[0]["messages"]
        system_msgs = [m for m in messages if m["role"] == "system"]
        return system_msgs[0]["content"] if system_msgs else ""

    async def test_auto_focus_false_by_default(self):
        llm = SequentialMockLLM(["Done."])
        agent = Agent(name="X", llm_provider=llm, tools=[_make_focus_tool()])
        await agent.run("query something")
        content = self._get_system_content(llm)
        assert "Focus DSL" not in content

    async def test_auto_focus_injects_system_prompt(self):
        llm = SequentialMockLLM(["Done."])
        agent = Agent(name="X", llm_provider=llm, tools=[_make_focus_tool()], auto_focus=True)
        await agent.run("query something")
        content = self._get_system_content(llm)
        assert "Focus DSL" in content

    async def test_auto_focus_names_the_tool(self):
        llm = SequentialMockLLM(["Done."])
        agent = Agent(name="X", llm_provider=llm, tools=[_make_focus_tool("my_query")], auto_focus=True)
        await agent.run("query something")
        content = self._get_system_content(llm)
        assert "my_query" in content

    async def test_auto_focus_includes_syntax_examples(self):
        llm = SequentialMockLLM(["Done."])
        agent = Agent(name="X", llm_provider=llm, tools=[_make_focus_tool()], auto_focus=True)
        await agent.run("query something")
        content = self._get_system_content(llm)
    ...
    async def test_auto_focus_no_injection_when_no_focus_tools(self):
        # A tool without `focus` in its schema should not trigger the instruction block
        async def h(args):
            return "ok"

        tool = AgentTool(
            name="plain_tool",
            description="No focus param",
            parameters={"type": "object", "properties": {"x": {"type": "string"}}, "required": []},
            handler=h,
        )
        llm = SequentialMockLLM(["Done."])
        agent = Agent(name="X", llm_provider=llm, tools=[tool], auto_focus=True)
        await agent.run("do something")
        content = self._get_system_content(llm)
        assert "Focus DSL" not in content

    async def test_auto_focus_lists_multiple_focus_tools(self):
        llm = SequentialMockLLM(["Done."])
        agent = Agent(
            name="X",
            llm_provider=llm,
            tools=[_make_focus_tool("tool_a"), _make_focus_tool("tool_b")],
            auto_focus=True,
        )
        await agent.run("query something")
        content = self._get_system_content(llm)
    ...
    async def test_auto_focus_coexists_with_developer_focus(self):
        # auto_focus=True + developer focus set — both stored, no conflict
        llm = SequentialMockLLM(["Done."])
        agent = Agent(
            name="X",
            llm_provider=llm,
            tools=[_make_focus_tool()],
            auto_focus=True,
            focus={"query_data": "status == 'active' | LIMIT 10"},
        )
        assert agent.auto_focus is True
        assert agent.default_focus == {"query_data": "status == 'active' | LIMIT 10"}

    async def test_auto_focus_combined_with_dev_prompt(self):
        # auto_focus instructions and developer prompt both appear in system message
        llm = SequentialMockLLM(["Done."])
        agent = Agent(
            name="X",
            llm_provider=llm,
