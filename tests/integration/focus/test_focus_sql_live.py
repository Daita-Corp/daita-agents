...
Run with:
    OPENAI_API_KEY=sk-... pytest tests/integration/focus/test_focus_sql_live.py -v -s -m requires_llm
"""
...
SALES = [
    ...
    },
    {
        "sale_id": "SL002",
        "rep_id": "R11",
        "rep_name": "Carlos Ruiz",
        "product_line": "On-Prem",
        "region": "AMER",
        "amount": 45000.0,
        "quarter": "Q1",
        "deal_size": "large",
...
def _build_sqlite_db(data: list, table: str) -> sqlite3.Connection:
    ...
    for row in data:
        vals = [
            (1 if row[c] is True else 0 if row[c] is False else row[c]) for c in cols
        ]
        conn.execute(
...
class TestPostgreSQLFocusPushdown:
    ...
    async def test_sql_pushdown_and_execution(self):
        ...
        assert all(
            set(r.keys()) == {"order_id", "product", "amount"} for r in rows
        ...
        print(
            f"[PG tokens]   raw≈{raw_tok}  focused≈{focused_tok}  (-{round((1-focused_tok/raw_tok)*100,1)}%)"
        )
...
class TestMySQLFocusPushdown:
    ...
    async def test_sql_pushdown_and_execution(self):
        ...
        assert (
            len(rows) == 3
        ), f"Expected 3 sessions (S001, S004, S006), got {len(rows)}: {rows}"
