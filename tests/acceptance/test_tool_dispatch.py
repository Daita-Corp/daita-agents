...
async def test_direct_file_dispatch_preserves_authenticated_result(
    tmp_path: Path,
) -> None:
...
async def test_file_query_search_load_dispatch_preserves_result(
    tmp_path: Path,
) -> None:
    workspace = _workspace(tmp_path)
...
async def test_connected_schema_dispatch_uses_one_catalog_call(
    tmp_path: Path,
) -> None:
    workspace = _workspace(tmp_path)
...
async def test_postgresql_dispatch_returns_structured_failure_without_fallback(
    tmp_path: Path,
) -> None:
    workspace = _workspace(tmp_path)
    provider = ToolboxAwareMockModelProvider(
...
async def test_mixed_file_and_source_dispatch_keeps_results_separate(
    tmp_path: Path,
) -> None:
    workspace = _workspace(tmp_path)
    (workspace.root / "sales.csv").write_text(
...
async def test_files_only_scope_projects_and_dispatches_only_file_tools(
    tmp_path: Path,
) -> None:
    workspace = _workspace(tmp_path)
...
async def test_latest_log_dispatch_searches_before_bound_tail_read(
    tmp_path: Path,
) -> None:
    workspace = _workspace(tmp_path)
...
async def test_markdown_report_dispatch_creates_and_delivers_on_request(
    tmp_path: Path,
) -> None:
    workspace = _workspace(tmp_path)
...
async def test_text_edit_dispatch_uses_one_approved_bound_replacement(
    tmp_path: Path,
) -> None:
    workspace = _workspace(tmp_path)
...
async def test_maximum_catalog_dispatch_is_bounded_searchable_and_replacing(
    tmp_path: Path,
) -> None:
