    ...
    async def report(self, case_id, status, failure):
        ...
        return {
            ...
            "source_sha256": {
                ...
                for path in (
                    Path(__file__),
                    Path("tests/live/test_postgresql_live_llm.py"),
                    Path("tests/test_postgresql_write_release.py"),
                    Path("tests/_postgresql_write_release_support.py"),
                    *sorted(Path("src/daita").rglob("*.py")),
                )
            },
            "limits": {
                "steps": 14,
