...
from tests.llm._routing_support import registration, request
...
async def test_provider_permit_is_attempt_scoped_and_released_for_backoff_and_route_change():
    events: list[str] = []

    class TracedProvider(MockModelProvider):
        async def generate(self, request):
            events.append(f"generate:{self.provider_id}")
            return await super().generate(request)

    first = TracedProvider(
        (
            ModelProviderError(ProviderErrorCode.TIMEOUT),
            ModelProviderError(ProviderErrorCode.INVALID_REQUEST),
        ),
        provider_id="mock:first-permit",
    )
    second = TracedProvider(
        (ModelResponse(finish_reason=FinishReason.STOP, text="fallback"),),
        provider_id="mock:second-permit",
    )

    @asynccontextmanager
    async def permit(key, deadline):
        del deadline
        events.append(f"permit-enter:{key}")
        try:
            yield object()
        finally:
            events.append(f"permit-exit:{key}")

    async def admit(key, deadline):
        return permit(key, deadline)
    ...
    router = ModelRouter(
        (
            replace(
                registration(first),
                concurrency_key="account:first",
            ),
            replace(
                registration(second),
                concurrency_key="account:second",
            ),
        ),
        retry_policy=RetryPolicy(
    ...
    router.bind_provider_admission(admit)

    assert (await router.generate(request())).text == "fallback"
    assert events == [
        "permit-enter:account:first",
        "generate:mock:first-permit",
        "permit-exit:account:first",
        "backoff",
        "permit-enter:account:first",
        "generate:mock:first-permit",
        "permit-exit:account:first",
        "permit-enter:account:second",
        "generate:mock:second-permit",
        "permit-exit:account:second",
    ]


async def test_stream_provider_permit_covers_stream_cleanup():
    events: list[str] = []

    class TracedStreamingProvider(MockStreamingModelProvider):
        async def stream(self, request):
            ...
            try:
                ...
                events.append("stream-cleanup")

    provider = TracedStreamingProvider(())
    ...
    router = ModelRouter((registration(provider, streaming=True),))
    router.bind_provider_admission(admit)

    streamed = [event async for event in router.stream(request())]

    assert isinstance(streamed[-1], ModelStreamCompleted)
    assert events == [
        "permit-enter",
        "stream-start",
        "stream-cleanup",
