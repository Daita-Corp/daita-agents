...
| API-ROOT-06 | `postgresql`, `sqlite` | Built-in resource adapters and declared executors | `next/tests/contract/domains/data/test_relational_read_conformance.py`; `next/tests/acceptance/test_postgresql_catalog_journey.py` | cutover | replace |
...
| EXT-03 | `postgresql` | Replacement-candidate adapter through the same contracts; deterministic conformance is complete and real service acceptance is Phase 9 | cutover | replace |
...
| EXT-16 | `cli` | External CLI parsing may be retained, but Phase 6 ships a thin development CLI against `AgentHost` (`next/src/daita/cli.py`; `next/tests/unit/test_cli.py`; `next/tests/unit/hosting/test_local_server.py`) and Phase 9 integrates the production package | cutover | external integration |
