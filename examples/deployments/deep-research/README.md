...
- Explicit stage handoffs: plan → raw findings → synthesis → report
- `MemoryPlugin` can contribute runtime context and capabilities when enabled
...
| Agent | Input | Output | Tools / Plugins | Runtime Context |
|---|---|---|---|---|
| Orchestrator | Research query | Research plan (JSON) | `decompose_query`, MemoryPlugin | memory context |
| Web Researcher | Research plan | Raw findings (JSON) | WebSearchPlugin, MemoryPlugin | memory context |
| Analyst | Raw findings | Synthesis (JSON) | `extract_scope`, MemoryPlugin, LineagePlugin | memory + lineage context |
| Report Writer | Synthesis | Markdown report | `format_citation`, `build_report_structure`, MemoryPlugin | memory context |
