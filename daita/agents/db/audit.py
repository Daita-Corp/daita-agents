        ...
        try:
            ...
            agent._db_audit_log.append(
                _entry(
                    agent,
                    prompt,
                    _sanitize_tool_calls(result.get("tool_calls", [])),
                )
            )
            ...
            entry = _entry(
                agent,
                prompt,
                _sanitize_tool_calls(list(agent._tool_call_history)),
            )
            entry["error"] = str(exc)
            agent._db_audit_log.append(entry)
            raise
    ...
    async def _audited_stream(prompt: str, **kwargs: Any):
        ...
        async for event in original_stream(prompt, **kwargs):
            if event.type == EventType.TOOL_CALL:
                ...
                agent._db_audit_log.append(
                    _entry(agent, prompt, _sanitize_tool_calls(tool_calls_this_run))
                )
            elif event.type == EventType.ERROR:
                entry = _entry(agent, prompt, _sanitize_tool_calls(tool_calls_this_run))
...
def _entry(
    agent: "Agent", prompt: str, tool_calls: List[Dict[str, Any]]
) -> Dict[str, Any]:
    entry = {
    ...
    context = getattr(agent, "_db_last_context_metadata", {})
    if context:
