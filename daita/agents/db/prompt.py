...
LARGE_SCHEMA_TABLE_SUMMARY_LIMIT = 200
...
def build_prompt(
    ...
    * 81+ tables — summary only (name + row count + column count)
    ...
    if table_count == 0:
        ...
        shown_tables = tables[:LARGE_SCHEMA_TABLE_SUMMARY_LIMIT]
        ...
        omitted = len(tables) - len(shown_tables)
        if omitted > 0:
            lines.append(
                f"- ... {omitted} additional tables omitted from prompt summary"
            )
