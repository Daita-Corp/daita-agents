...
class TracedPlugin:
    ...
    def _prepare_input_data(self, args: tuple, kwargs: dict) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "method_type": (
                    "async"
                    if asyncio.iscoroutinefunction(args[0] if args else None)
                    else "sync"
                ),
            }
        except Exception as e:
    ...
    def _extract_result_metadata(self, result: Any) -> Dict[str, Any]:
        ...
        metadata = {"result_type": type(result).__name__, "success": True}
...
def get_plugin_traces(
    plugin_name: Optional[str] = None, limit: int = 20
) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        tool_ops = [op for op in operations if op.get("type") == "tool_execution"]
...
def create_custom_traced_plugin(
    plugin_instance: Any, plugin_name: str, custom_metadata: Dict[str, Any] = None
) -> TracedPlugin:
    """
