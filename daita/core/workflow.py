...
Example:
    ...
    fetcher = Agent(name="Data Fetcher")
...
class Workflow:
    ...
    def add_agent_pool(
        self, name: str, agent_factory: Any, instances: int = 1
    ) -> "Workflow":
        """
        ...
        logger.debug(
            f"Added agent pool '{name}' with {instances} instances to workflow '{self.name}'"
        )
        return self
    ...
    def remove_agent(self, name: str) -> bool:
        ...
        self.connections = [
            c for c in self.connections if c.from_agent != name and c.to_agent != name
        ]
    ...
    def connect(
        self, from_agent: str, channel: str, to_agent: str, task: str = "relay_message"
    ) -> "Workflow":
        """
    ...
    def configure_reliability(
        ...
        config = ReliabilityConfig(
            ...
            backpressure_control=(
                backpressure_control if backpressure_control is not None else True
            ),
        )
    ...
    def validate_connections(self) -> List[str]:
        ...
        for conn in self.connections:
            ...
            if (
                conn.from_agent not in self.agents
                and conn.from_agent not in self.agent_pools
            ):
                errors.append(f"Source '{conn.from_agent}' not found in workflow")

            # Check to_agent exists
            if (
                conn.to_agent not in self.agents
                and conn.to_agent not in self.agent_pools
            ):
            ...
            if conn.from_agent == conn.to_agent:
                errors.append(
                    f"Circular dependency: {conn.from_agent} -> {conn.to_agent}"
                )
    ...
    async def start(self) -> None:
        ...
        try:
            ...
            if validation_errors:
                raise WorkflowError(
                    f"Invalid connections: {'; '.join(validation_errors)}"
                )
            ...
            for pool_name, pool in self.agent_pools.items():
                try:
                    await pool.start()
                    logger.debug(
                        f"Started agent pool '{pool_name}' with {pool.instance_count} instances"
                    )
                except Exception as e:
                    logger.error(f"Failed to start agent pool '{pool_name}': {str(e)}")
                    raise WorkflowError(
                        f"Failed to start agent pool '{pool_name}': {str(e)}"
                    )
            ...
            await self._trace_workflow_event(
                "workflow_error", {"workflow_name": self.name, "error": str(e)}
            )
            raise
    ...
    async def stop(self) -> None:
        ...
        try:
            ...
            await self._trace_workflow_event(
                ...
                {
                    "workflow_name": self.name,
                    "running_time_seconds": self.stopped_at
                    - (self.started_at or self.stopped_at),
                },
            )
    ...
    async def _setup_connections(self) -> None:
        ...
        for connection in self.connections:
            try:
                ...
                logger.debug(
                    f"Set up connection: {connection} (reliability: {self._reliability_enabled})"
                )

            except Exception as e:
                logger.error(f"Failed to set up connection {connection}: {str(e)}")
                raise WorkflowError(
                    f"Failed to set up connection {connection}: {str(e)}"
                )
            ...
            try:
                ...
                if connection.to_agent in self.agents:
                    ...
                    # Fallback to old API for legacy agents
                        ...
                        await dest_agent._process(
                            connection.task, data, enriched_context
                        )
                    else:
                        logger.warning(
                    ...
                    logger.error(
                        f"Destination '{connection.to_agent}' not found in agents or agent pools"
                    )
    ...
    async def _configure_agents_reliability(self) -> None:
        ...
        for agent_name, agent in self.agents.items():
            try:
                ...
                if (
                    hasattr(agent, "enable_reliability_features")
                    and self.reliability_config.task_tracking
                ):
                    agent.enable_reliability_features()
                    logger.debug(
                        f"Enabled reliability features for agent '{agent_name}'"
                    )

                # Configure backpressure if supported
                if (
                    hasattr(agent, "backpressure_controller")
                    and self.reliability_config.backpressure_control
                    and agent.backpressure_controller is None
                ):
                    ...
                    logger.debug(
                        f"Configured backpressure control for agent '{agent_name}'"
                    )

            except Exception as e:
                logger.warning(
                    f"Failed to configure reliability for agent '{agent_name}': {e}"
                )
            ...
            if message_id:
                if message_id in self._processed_messages:
                    ...
                    if (
                        ...
                        and self.reliability_config.acknowledgments
                    ):
            ...
            try:
                ...
                if hasattr(dest_agent, "receive_message"):
                    ...
                    if (
                        self.reliability_config
                        and self.reliability_config.backpressure_control
                        and hasattr(dest_agent, "backpressure_controller")
                        and dest_agent.backpressure_controller
                    ):

                        # Check if agent can handle the task
                        if (
                            not await dest_agent.backpressure_controller.acquire_processing_slot()
                        ):
                            raise BackpressureError(
                                f"Agent {connection.to_agent} queue is full",
                                agent_id=getattr(
                                    dest_agent, "agent_id", connection.to_agent
                                ),
                            )
                ...
                # Fallback to old API for legacy agents
                    ...
                    if (
                        ...
                        await self.relay_manager.nack_message(
                            message_id,
                            "Agent has no receive_message() or _process() method",
                        )
                ...
                logger.error(
                    f"Error in reliable workflow communication {connection}: {str(e)}"
                )
    ...
    async def _cleanup_connections(self) -> None:
        ...
        for channel, callback in self._subscriptions:
            try:
                ...
                logger.warning(
                    f"Error cleaning up subscription for channel '{channel}': {str(e)}"
                )
    ...
    async def inject_data(
        self, agent_name: str, data: Any, task: str = "inject"
    ) -> None:
        """
        ...
        if self.status != WorkflowStatus.RUNNING:
            raise WorkflowError(
                f"Cannot inject data - workflow is not running (status: {self.status})"
            )
        ...
        try:
            ...
            if hasattr(agent, "process"):
                await agent.process(
                    task, data, {"workflow": self.name, "injection": True}
                )
                logger.debug(
                    f"Injected data into agent '{agent_name}' in workflow '{self.name}'"
                )
            else:
    ...
    async def _trace_workflow_event(
        ...
        try:
            span_id = self.trace_manager.start_span(
                ...
                metadata={"workflow_name": self.name, "event_type": event_type},
            )
        ...
        if hasattr(agent, "enable_reliability") and agent.enable_reliability:
                ...
                try:
                    ...
                    for task in tasks:
                        ...
                        stats["tasks_by_status"][status] = (
                            stats["tasks_by_status"].get(status, 0) + 1
                        )
                except Exception as e:
                    logger.warning(
                        f"Failed to get task stats for agent {agent_name}: {e}"
                    )
            ...
            if hasattr(agent, "get_backpressure_stats"):
                try:
                    ...
                    logger.warning(
                        f"Failed to get backpressure stats for agent {agent_name}: {e}"
                    )
                ...
                if hasattr(agent, "get_token_usage"):
                    ...
                    if usage and isinstance(usage, dict):
                        ...
                        if tokens > 0:
                            ...
                            total_usage["prompt_tokens"] += usage.get(
                                "prompt_tokens", 0
                            )
                            total_usage["completion_tokens"] += usage.get(
                                "completion_tokens", 0
                            )
                            total_usage["llm_calls"] += usage.get(
                                "total_calls", usage.get("llm_calls", 0)
                            )
                            total_usage["agents_with_usage"].append(agent_name)
                            logger.debug(
                                f"Workflow: Agent '{agent_name}' used {tokens} tokens"
                            )

                # Method 2: Check if agent has llm_provider with usage tracking
                elif hasattr(agent, "llm_provider") and hasattr(
                    agent.llm_provider, "get_token_usage"
                ):
                    ...
                    if llm_usage and isinstance(llm_usage, dict):
                        ...
                        if tokens > 0:
                            total_usage["total_tokens"] += tokens
                            total_usage["prompt_tokens"] += llm_usage.get(
                                "prompt_tokens", 0
                            )
                            total_usage["completion_tokens"] += llm_usage.get(
                                "completion_tokens", 0
                            )
                            total_usage["llm_calls"] += 1
                            total_usage["agents_with_usage"].append(agent_name)
                            logger.debug(
                                f"Workflow: Agent '{agent_name}' (via llm_provider) used {tokens} tokens"
                            )

            except Exception as e:
                logger.warning(
                    f"Failed to get token usage from agent '{agent_name}': {e}"
                )

        if total_usage["total_tokens"] > 0:
            logger.info(
                f"Workflow '{self.name}' total token usage: {total_usage['total_tokens']} tokens across {len(total_usage['agents_with_usage'])} agents"
            )
            return total_usage
        else:
        ...
        for agent_name, agent in self.agents.items():
            ...
            if not agent_health["has_receive_message"]:
                health["issues"].append(
                    f"Agent '{agent_name}' has no receive_message method"
                )
