...
class DecisionRecorder:
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        try:
            if self.span_id:
                ...
                logger.debug(
                    f"Ended decision trace: {self.decision_point} (confidence: {self.context.confidence_score:.2f})"
                )
    ...
    def set_confidence(self, confidence: float):
        ...
        self._emit_event(
            DecisionEventType.CONFIDENCE_UPDATED,
            {
    ...
    def add_reasoning(self, reasoning: str):
        ...
        self._emit_event(
            DecisionEventType.REASONING_ADDED,
            {
                "reasoning": reasoning,
    ...
    def add_alternative(self, alternative: str):
        ...
        self._emit_event(
            DecisionEventType.ALTERNATIVE_ADDED,
            {
...
async def record_decision_point(
    ...
    recorder = DecisionRecorder(
        decision_point, decision_type, agent_id, stream_callback
    )
        ...
        if asyncio.iscoroutinefunction(func):
            ...
            async def async_wrapper(*args, **kwargs):
                ...
                async with record_decision_point(
                    decision_point, decision_type, agent_id
                ) as decision:
                    try:
                ...
                async def async_exec():
                    ...
                    async with record_decision_point(
                        ...
                        try:
                            ...
                            logger.error(
                                f"Decision function {func.__name__} failed: {e}"
                            )
                            raise
...
def _extract_decision_metadata(
    result, extract_confidence: bool, extract_reasoning: bool
...
async def record_classification_decision(
    ...
    return await record_decision(
        ...
        factors=feature_weights,
        factor_prefix="weight_",
...
def get_recent_decisions(
    agent_id: Optional[str] = None, decision_type: Optional[str] = None, limit: int = 20
) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        operations = trace_manager.get_recent_operations(
            agent_id=agent_id, limit=limit * 2
        )

        # Filter for decision traces
        decisions = [op for op in operations if op.get("type") == "decision_trace"]
...
def register_agent_decision_stream(
    agent_id: str, callback: Callable[[DecisionEvent], None]
) -> None:
    """
...
def unregister_agent_decision_stream(
    agent_id: str, callback: Callable[[DecisionEvent], None]
) -> None:
    """
...
def get_decision_stats(
    agent_id: Optional[str] = None, decision_type: Optional[str] = None
) -> Dict[str, Any]:
    ...
    try:
        ...
        successful_decisions = len(
            [d for d in decisions if d.get("status") == "success"]
        )
