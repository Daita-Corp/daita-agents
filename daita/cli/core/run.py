    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            async with session.post(
                ...
                if follow:
                    return await _follow_execution(session, api_base, headers, execution_id, display_name, verbose)
                else:
                    return await _poll_for_result(session, api_base, headers, execution_id, display_name, verbose)
...
async def _await_execution(
    ...
    anim_task = asyncio.create_task(_spinner_loop(state, agent_name, c)) if ansi else None
        ...
        while time.time() < deadline:
            ...
            async with session.get(
                ...
                if not ansi and status != last_plain_status:
                    click.echo(f"  [{_fmt_elapsed(time.time() - state['start'])}] {status}")
...
async def _follow_execution(
    ...
    return await _await_execution(
        session, api_base, headers, execution_id, agent_name, verbose,
        poll_interval=2.0, max_wait_s=900,
    )
...
async def _poll_for_result(
    ...
    return await _await_execution(
        session, api_base, headers, execution_id, agent_name, verbose,
        poll_interval=0.5, max_wait_s=360,
    )
