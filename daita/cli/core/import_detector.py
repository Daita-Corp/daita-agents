...
class ImportDetector:
    ...
    def __init__(self):
        ...
        self.layer_packages = {
            "common_dependencies": {
                "requests",
                "dateutil",
                "python_dateutil",  # python-dateutil can be imported as either
                "PIL",
                "pillow",  # Pillow can be imported as PIL
                "tqdm",
                "joblib",
            },
            "core_dependencies": {
    ...
    def get_layer_arns_for_project(
        self, project_path: Path, layer_config_path: Path
    ) -> List[str]:
        ...
        for layer_name in required_layers.keys():
            if layer_name in layer_arn_mapping and layer_arn_mapping[layer_name]:
                ...
                logger.info(
                    f" Including {layer_name} layer: {layer_arn_mapping[layer_name]}"
                )
...
def main():
    ...
    print(
        f" Optimization potential: {'Yes' if analysis['optimization_potential'] else 'No'}"
    )
