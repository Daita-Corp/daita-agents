...
async def list_deployments(
    ...
    try:
        ...
        print(
            f" Deployment History{' for ' + repr(project_name) if project_name else ''}"
        )
        if environment:
            ...
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    for i, deployment in enumerate(deployments[:limit], 1):
                        status_emoji = (
                            "●" if deployment.get("status") == "active" else "○"
                        )
                        ...
                        try:
                            ...
                            deployed_str = deployed_date.strftime(
                                "%Y-%m-%d %H:%M:%S UTC"
                            )
                        except:
                        ...
                        print(
                            f"      Environment: {deployment.get('environment', 'unknown')}"
                        )
                        ...
                        if "package_size_bytes" in deployment:
                            package_size_mb = (
                                deployment["package_size_bytes"] / 1024 / 1024
                            )
                        ...
                        if deployment_info and "functions" in deployment_info:
                            agent_names = [
                                f.get("agent_name", "Unknown")
                                for f in deployment_info["functions"]
                            ]
                            if agent_names:
...
async def show_deployment_details(deployment_id: str):
    ...
    try:
        ...
        deployed_date = datetime.fromisoformat(
            target_deployment["deployed_at"].replace("Z", "+00:00")
        )
        ...
        print(
            f"Package Size: {target_deployment.get('package_size_bytes', 0) / 1024 / 1024:.1f}MB"
        )
        print()
        ...
        if agents:
            ...
            for agent in agents:
                ...
                print(f"      Type: {agent.get('type', 'standard')}")
...
async def rollback_deployment(deployment_id: str, environment: str = "production"):
    ...
    try:
        ...
        if environment == "production":
            confirm = input(
                "  Rollback PRODUCTION environment? Type 'yes' to confirm: "
            )
            if confirm != "yes":
        ...
        async with aiohttp.ClientSession() as session:
            ...
            async with session.delete(url, headers=headers, timeout=30) as response:
                if response.status == 200:
                    ...
                    print(
                        f" {result.get('message', 'Deployment deleted successfully')}"
                    )
