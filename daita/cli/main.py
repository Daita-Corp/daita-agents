...
from .core.deployments import list_deployments, show_deployment_details, delete_deployment
...
@click.option('--follow', '-f', is_flag=True, help='Follow execution progress in real-time')
...
def run(ctx, target_name, target_type, data_file, data_json, task, follow, timeout):
    ...
    try:
        success = asyncio.run(run_remote_execution(
...
def list_executions(ctx, limit, status, target_type):
    ...
    try:
        success = asyncio.run(list_remote_executions(
            limit=limit,
            status=status,
            target_type=target_type,
            environment='production',
...
def execution_logs(ctx, execution_id, follow):
    ...
    try:
        success = asyncio.run(get_execution_logs(
...
def memory_status(ctx, project):
    ...
    try:
        ...
        asyncio.run(show_memory_status(
            project=project,
            verbose=ctx.obj.get('verbose', False)
        ))
    except KeyboardInterrupt:
...
def memory_show(ctx, workspace, full, limit, project):
    ...
    try:
        ...
        asyncio.run(show_workspace_memory(
            workspace=workspace,
            full=full,
            limit=limit,
            project=project,
...
def secrets_set(ctx, key, value):
    ...
    try:
        ...
        success = asyncio.run(set_secret(key, value, verbose=ctx.obj.get('verbose', False)))
        if not success:
            sys.exit(1)
    except Exception as e:
        if ctx.obj.get('verbose'):
...
def secrets_import(ctx, env_file):
    ...
    try:
        ...
        success = asyncio.run(import_env(env_file, verbose=ctx.obj.get('verbose', False)))
        if not success:
            sys.exit(1)
    except Exception as e:
        if ctx.obj.get('verbose'):
