...
from .elasticsearch_extensions import (
    ELASTICSEARCH_MANIFEST,
    ElasticsearchExecutor,
    elasticsearch_capabilities,
    elasticsearch_evidence_schemas,
    elasticsearch_operation_definitions,
    elasticsearch_tool_views,
)
...
class ElasticsearchPlugin(ConnectorPlugin):
    ...
    async def setup(self, context: PluginContext) -> None:
        """Set up the Elasticsearch connector for a runtime."""
        await self.connect()

    async def teardown(self) -> None:
        """Disconnect the Elasticsearch connector from a runtime."""
    ...
    def declare_capabilities(self):
        return elasticsearch_capabilities(self.read_only)

    def get_executors(self):
        return (self._executor,)

    def declare_evidence_schemas(self):
        return elasticsearch_evidence_schemas()

    def get_tool_views(self):
        return elasticsearch_tool_views(self.read_only)

    def _definition_for_capability(self, capability_id: str) -> dict:
        for definition in elasticsearch_operation_definitions(self.read_only):
    ...
    async def search(
        ...
        **kwargs,
        ...
        try:
            ...
            search_params = {"index": index, "body": search_body, **kwargs}
            ...
            logger.info(
                f"Search completed: {result['hits']['total']['value']} hits in {result['took']}ms"
            )
            return result
        ...
        try:
            ...
            def doc_generator():
                for doc in documents:
                    yield {"_index": index, "_source": doc, **kwargs}
    ...
    async def search_agent_logs(
        ...
        return await self.search(
            index=index, query=query, focus=focus, size=size, sort=sort
        )
    ...
    async def analyze_performance(
        ...
        # Build aggregations
        aggregations = {
            "performance_stats": {"stats": {"field": metric_field}},
    ...
    async def _tool_index(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await self.index_document(
            index=index, document=document, doc_id=doc_id
        )
    ...
    async def _tool_bulk_index(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await self.bulk_index(
            index=index, documents=documents, batch_size=batch_size
        )
    ...
    async def _tool_create_index(self, args: Dict[str, Any]) -> Dict[str, Any]:
        ...
        result = await self.create_index(
            index=index, mapping=mapping, settings=settings
        )
