...
class AWSDiscoverer(BaseDiscoverer):
    ...
    Enumerates RDS, DynamoDB, S3, ElastiCache, Redshift, API Gateway,
    SQS, SNS, OpenSearch, DocumentDB, and Kinesis.
    ...
    # Maps service name -> enumerate method name
    _SERVICE_METHODS = {
        "rds": "_enumerate_rds",
        "dynamodb": "_enumerate_dynamodb",
        "s3": "_enumerate_s3",
        "elasticache": "_enumerate_elasticache",
        "redshift": "_enumerate_redshift",
        "apigateway": "_enumerate_apigateway",
        "sqs": "_enumerate_sqs",
        "sns": "_enumerate_sns",
        "opensearch": "_enumerate_opensearch",
        "documentdb": "_enumerate_documentdb",
        "kinesis": "_enumerate_kinesis",
    }
    ...
    async def enumerate(self) -> AsyncIterator[DiscoveredStore]:
        ...
        for region in self._regions:
            for service in self._services:
                if method_name := self._SERVICE_METHODS.get(service):
                    async for store in getattr(self, method_name)(region):
                        yield store
    ...
    def _build_store(
        ...
        """Build a DiscoveredStore with fingerprint."""
            ...
            for page in paginator.paginate():
                for table_name in page.get("TableNames", []):
                    ...
                    yield self._build_store(
                        ...
                        connection_hint={"table_name": table_name, "region": region},
                        source="aws_dynamodb",
                        region=region,
                        metadata={
                            "arn": arn,
        ...
        try:
            for bucket in client.list_buckets().get("Buckets", []):
                ...
                yield self._build_store(
                    ...
                    connection_hint={"bucket": bucket_name, "region": bucket_region},
                    source="aws_s3",
        ...
        try:
            ...
            for page in paginator.paginate(ShowCacheNodeInfo=True):
                for cluster in page.get("CacheClusters", []):
                    ...
                    ep = nodes[0].get("Endpoint", {}) if nodes else {}

                    yield self._build_store(
                        store_type=cluster.get("Engine", "redis"),
            ...
            for page in paginator.paginate():
                for queue_url in page.get("QueueUrls", []):
                    ...
                    yield self._build_store(
                        ...
                        connection_hint={"queue_url": queue_url, "region": region},
                        source="aws_sqs",
                        region=region,
                        metadata={
                            "arn": arn,
            ...
            for page in paginator.paginate():
                for topic in page.get("Topics", []):
                    ...
                    yield self._build_store(
                        ...
                        connection_hint={"topic_arn": topic_arn, "region": region},
                        source="aws_sns",
                        region=region,
                        metadata={
                            "arn": topic_arn,
            ...
            for page in paginator.paginate():
                for stream_name in page.get("StreamNames", []):
                    ...
                    yield self._build_store(
                        ...
                        connection_hint={"stream_name": stream_name, "region": region},
                        source="aws_kinesis",
                        region=region,
                        metadata={
                            "arn": arn,
