...
class Settings(BaseModel):
    ...
    def __init__(self, **data):
        ...
        if (
            ...
            or os.getenv("DAITA_RUNTIME") == "lambda"
        ):
        ...
        if api_endpoint:
            try:
                ...
                raise ValueError(
                    f"Invalid DAITA_API_ENDPOINT environment variable: {e}"
                )
        ...
        if dashboard_url:
            try:
                ...
                raise ValueError(
                    f"Invalid DAITA_DASHBOARD_URL environment variable: {e}"
                )
        ...
        if os.getenv("DAITA_RUNTIME") == "lambda":
            if not self.api_endpoint:
                raise ValueError(
                    "DAITA_API_ENDPOINT environment variable required for Lambda runtime"
                )
            if not self.dashboard_url:
                raise ValueError(
                    "DAITA_DASHBOARD_URL environment variable required for Lambda runtime"
                )
